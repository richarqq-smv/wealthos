 ...
 *
 * `.web.ts` on purpose: pdfjs-dist's worker loading uses a browser-oriented
 * dynamic `import()` that Hermes's bytecode compiler rejects outright at
 * Android build time ("Invalid expression encountered") — confirmed via a
 * real EAS Android build failure, not a guess. Metro/Expo's platform-
 * extension resolution means this file is only ever bundled for web (which
 * also covers the Electron desktop build, itself a wrapped web export) —
 * see the native stub `pdfExtract.native.ts` for Android/iOS.
 */
